...
class PagerdutyHandler(SensuHandler):
	...
	def api_key(self):
		...
		if 'pager_team' in event['check'] and event['check']['pager_team'] is not None:
			...
			try:
				...
				logging.warning(
					"HANDLER: your check config has a pager team of %s but the team was not found in the handler config, using default key.",
					pager_team)
				api_key = settings[self.config]['api_key']
			...
			try:
				...
				logging.warning(
					"HANDLER: your client config has a pager team of %s but the team was not found in the handler config, using default key.",
					pager_team)
				api_key = settings[self.config]['api_key']
		else:
	...
	def contexts(self):
		...
		try:
			...
			event_output = json.loads(event['check']['output'])
		try:  # If event_output is a string convert to dictionary
			outputs = json.loads(event_output)
		except ValueError:  # If event_output is already a dictionary
			outputs = event_output
		except TypeError:  # If event_output can not be converted to a dictionary
			outputs = event_output
		except BaseException as e:  # Catch all and throw raw exception to the log
			logging.error(e)
	...
	def description_prefix(self):
		# dynamic_description_prefix_key is attempting to match a key from the client config which is passed into the
		# event data. If this key doesn't exist we try to set it to the static prefix defined in the server side
		# config (i.e. conf.d/pagerduty.json) -- using the key 'description_prefix'.  This can be any static value you
		# want to have in front of the event summary in pagerduty.
	...
	def handle(self):
		settings = self.grab_settings()
		event = self.grab_event()
		try:  # If event check output is a dictionary
			event_output = event['check']['output']
		except TypeError:  # If event check output is a string
			event_output = json.loads(event['check']['output'])

		try:  # If event_output is a string
			outputs = json.loads(event_output)
		except ValueError:  # If event_output is already a dictionary
			outputs = event_output
		except TypeError:  # If event_output can not be converted to a dictionary
		...
		if settings[self.config] is None:
			logging.error('HANDLER: invalid config: {settings[self.config] !r} you need to pass a key and not a file.')
		...
		if proxy_settings['proxy_host']:
			...
			if event['action'] == 'create' or event['action'] == 'flapping':
				...
				if type(links) is str:
					...
					if '{' not in links:
						if 'href' not in links:
							...
							elif links.find('=') >= 4 and links.find(
								'=') <= 6:  # If an equals sign is immediately after href
								s = links.split("=")
								links = _links.append({'href': s[1]})
							else:
								# If href at the beginning but we don't know what's after we just push links back
								# into the value without href
				...
				try:
					...
					logging.warning(
						'HANDLER: Severity must be "critical", "error", "warning", or "info"... \nSetting severity to "error".')
					severity = 'error'
				try:
				...
				try:
					...
					logging.error(
						"HANDLER: A service email (svc_email) must be set in the handler config JSON on your server.\n"
